...
: ${_CONF_DEV_JAVA_TIMO_A_LOMBOKER_JDK:=java}
: ${_CONF_DEV_JAVA_TIMO_A_LOMBOKER_VERSION:=0.0.0}
: ${_CONF_DEV_JAVA_TIMO_A_LOMBOKER_JAR_NAME:=lomboker-$_CONF_DEV_JAVA_TIMO_A_LOMBOKER_VERSION.jar}
: ${_CONF_DEV_JAVA_TIMO_A_LOMBOKER_JAR:=$_CONF_INSTALL_APPLICATION_LIBRARY_PATH/$_CONF_DEV_JAVA_TIMO_A_LOMBOKER_JAR_NAME}
_beep() {
	if [ -n "$_BEEPING" ]; then
...
xargs -P$_CONF_DEV_FORMAT_PARALLEL $_CONF_INSTALL_APPLICATION_LIBRARY_PATH/action/format/java/bin/lombok/.timo-a-lomboker
case $_DETECTED_PLATFORM in
$_PLATFORM) ;;
