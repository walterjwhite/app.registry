...
. $_CONF_INSTALL_APPLICATION_LIBRARY_PATH/action/format/java/.sorald.conf
...
if [ -e "$_SORALD_JAR" ]; then
	_SORALD_ENABLED=0
fi
_sorald_fix() {
	local _rule=$1
	local _file=$2
	java -Dos.name=Linux -jar $_SORALD_JAR repair --rule-key=$_rule --source=$_file
}
_sorald_fix_all() {
...
[ $_SORALD_ENABLED ] && _sorald_fix_all || {
	_warn "Sorald is disabled"
}
