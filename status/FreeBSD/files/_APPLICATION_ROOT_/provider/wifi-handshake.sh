...
_WIFI_HANDSHAKE_LOG_FILE=/var/log/messages/log.0.zst
_wifi_handshake() {
	local message
	local device
	local instance
	for device in $(zstdgrep 'hostapd' $_WIFI_HANDSHAKE_LOG_FILE | $_CONF_INSTALL_GNU_GREP -P \
		...
		$_CONF_INSTALL_GNU_GREP -Po '[a-f0-9]{2}\:[a-f0-9]{2}\:[a-f0-9]{2}\:[a-f0-9]{2}\:[a-f0-9]{2}\:[a-f0-9]{2}' | sort -u); do
