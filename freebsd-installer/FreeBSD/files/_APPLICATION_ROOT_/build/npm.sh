...
_npm() {
	...
	if [ -n "$_VARIANT" ]; then
		variant_options="-or -path \"*/*.patch/variants/$_VARIANT/npm\""
	fi
	if [ $(find patches -type f -path '*/*.patch/npm' $variant_options -exec $_CONF_INSTALL_GNU_GREP -Pv '(^#|^$)' {} \; | sort -u | wc -l) -eq 0 ]; then
		_warn "No npm packages found"
		return 1
	fi
	_pkg_install npm
	find patches -type f -path '*/*.patch/npm' $variant_options -exec $_CONF_INSTALL_GNU_GREP -Pv '(^#|^$)' {} \; | sort -u | tr '\n' ' ' | xargs npm install -g
}
