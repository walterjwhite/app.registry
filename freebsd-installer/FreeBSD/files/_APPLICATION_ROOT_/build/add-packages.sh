...
_add_packages() {
	...
	if [ -n "$_VARIANT" ]; then
		variant_options="-or -path \"*/*.patch/variants/$_VARIANT/add-packages\""
	fi
	local packages_to_install=$(find patches -type f -path '*/*.patch/add-packages' $variant_options -exec $_CONF_INSTALL_GNU_GREP -Pv '(^#|^$)' {} \; | sort -u | tr '\n' ' ')
	local install_output
	install_output=$(_add_packages_install $packages_to_install)
	local install_status=$?
	printf '%s\n' "$install_output"
	if [ $install_status -gt 0 ]; then
		local missing_packages=$(printf '%s\n' "$install_output" | $_CONF_INSTALL_GNU_GREP -Po "pkg: No packages available to install matching '.*' have been found in the repositories" |
			$_CONF_INSTALL_GNU_GREP -Po "'.*'" | tr -d "'" | sort -u | tr '\n' '|' | sed -e 's/|$//')
		_warn "These packages are unavailable: $missing_packages"
		packages_to_install=$(printf '%s\n' $packages_to_install | tr ' ' '\n' | $_CONF_INSTALL_GNU_GREP -Pv "^($failed_packages)$" | tr '\n' ' ')
		_add_packages_install $packages_to_install
	fi
}
_add_packages_install() {
	_CONF_INSTALL_STEP_TIMEOUT=1800 _pkg_install $@
}
